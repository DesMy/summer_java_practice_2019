...
public class AlgorithmTest 
{
	...
	@Test
	public void testZeroGraph() {
		try {
            FordFulkerson.process(target, 'a', 'f');
            assertTrue(false);
        }
        catch(Exception ex) {
	...
	public void testNoSource() {
		target.addPair('a', 'b', 7);
		try {
            FordFulkerson.process(target, 'a', 'c');
            assertTrue(false);
        }
        catch(Exception ex) {
            assertEquals(ex.getMessage(), "Wrong source or sink.");
	...
	public void testNoSink() {
		target.addPair('a', 'b', 7);
		try {
            FordFulkerson.process(target, 'e', 'b');
            assertTrue(false);
        }
        catch(Exception ex) {
            assertEquals(ex.getMessage(), "Wrong source or sink.");
	...
	public void testOneVertex() {
		target.addPair('a', 'b', 1);
		try {
			FordFulkerson.process(target, 'a', 'a');
			assertTrue(false);
		}
		catch(Exception ex) {
			assertEquals(ex.getMessage(), "Wrong source or sink.");
